...
class TrafficSpec(object):
    def __init__(self, loss_rate=None, pps=1e6, pkt_size=60, num_flows=10,
                 flow_duration=1, flow_rate=None, arrival='uniform',
                 duration='uniform', latency=False):
        self.pps = pps
        self.loss_rate = loss_rate # dynamic tx will be used if not None
...
class Session(object):
    ...
    def adjust_tx_rate(self):
        if self.__spec.loss_rate is None:
            return
